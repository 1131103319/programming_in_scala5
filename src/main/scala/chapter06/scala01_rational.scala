...
object scala01_rational extends App {
   ...
   * 最后，不可变对象可以被安全地用作哈希表里的键。
   * 举例来说，如果某个可变对象在被添加到HashSet以后改变了，则当你下次再检索该HashSet的时候，可能就找不到这个对象了
   *
  ...
  class Rational(n: Int, d: Int) {
    //println("Created " + n + "/" + d)
    //require方法接收一个Boolean类型的参数。
    // 如果传入的参数为true，则require方法将会正常返回;否则,require方法将会抛出lllegalArgumentException来阻止对象的构建。
    require(d != 0)
  ...
  /**
   * 当你实例化那些接收参数的类（如Rational类)时，可以选择不new关键字。
   * 这样的代码编写方式被称作“通用应用方法”(universal apply method)。例如:
   */
