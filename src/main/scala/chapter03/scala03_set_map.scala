...
object scala03_set_map extends App {
   ...
   * 举例来说，数组永远是可变的;列表永远是不可变的。Scala还提供了集（set）和映射(map）的可变和不可变的不同选择，但使用同样的简称。
   * 对集和映射而言，Scala通过不同的类继承关系来区分可变和不可变版本。
   *  例如，Scala的API包含了一个基础的特质来表示集，
   * 这里的特质与Java的接口定义类似。(你将在第11章了解到更多关于特质的内容。)
   * 在此基础上，Scala提供了两个子特质(subtrait)，一个用于表示可变集,另一个用于表示不可变集。
   */
